...
func get_discovered_rooms() -> Array[Dictionary]:
	return discovery.get_rooms()
...
func sync_advertise() -> void:
	if current_room == null or not ConnectionManager.is_server() or not current_room.is_public:
		...
		return
	var payload: Dictionary = discovery.build_advertise_payload(current_room)
	if discovery.is_advertising():
		discovery.update_payload(payload)
	else:
		discovery.start_advertising(payload)
